...
// ============================================
// Replicate Translation API endpoints
// ============================================
...
// Создать новый Replicate Translation сервис
app.post("/api/replicate-translation", express.json(), async (req, res) => {
  try {
    ...
    // Проверяем, существует ли whisperx relay
    ...
    translator.on("translation_start", (data) => {
      broadcastToReplicateTranslatorClients(translatorId, {
    ...
    res.json({
      ...
      contextSize: contextSize || 10,
      status: "running",
      createdAt: replicateTranslators.get(translatorId).createdAt
    });
...
// Получить список всех Replicate Translation сервисов
...
// Получить статус конкретного Replicate Translation сервиса
app.get("/api/replicate-translation/:id", (req, res) => {
  ...
  res.json({
    id,
    whisperxId: translatorData.whisperxId,
    model: translatorData.model,
    status: translatorData.status,
    createdAt: translatorData.createdAt,
    lastError: translatorData.lastError || null,
    details: status
...
// Остановить и удалить Replicate Translation сервис
...
app.get("/api/replicate-translation/:id/logs", (req, res) => {
  ...
  let logs;
  if (type) {
    logs = translatorData.translator.getLogsByType(type, limit);
  } else {
    logs = translatorData.translator.getLogs(limit);
  }
...
// Ручной перевод текста (для тестирования)
